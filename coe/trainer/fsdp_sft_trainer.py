...
class FSDPSFTTrainer(object):
    ...
    def __init__(self, config, device_mesh: DeviceMesh, ulysses_device_mesh: DeviceMesh, dataset_class = SFTDataset):
        self.config = config
    ...
    def _get_model(self, local_model_path, config, trust_remote_code):
        model = AutoModelForCausalLM.from_pretrained(local_model_path, config=config, torch_dtype=torch.float32, attn_implementation='flash_attention_2', trust_remote_code=trust_remote_code)
        return model

    def _build_dataset(self):
        config = self.config
        # build dataset
        self.train_dataset = self.dataset_class(parquet_files=config.data.train_files,
        ...
        self.val_dataset = self.dataset_class(parquet_files=config.data.val_files,
                                      tokenizer=self.tokenizer,
                                      prompt_key=config.data.prompt_key,
                                      prompt_dict_keys=config.data.get('prompt_dict_keys', None),
                                      response_key=config.data.response_key,
    ...
    def _build_model_optimizer(self):
        ...
        with init_context():
            self.model: PreTrainedModel = self._get_model(local_model_path, config, trust_remote_code)
        ...
        self._build_lr_scheduler(num_warmup_steps)


    def _build_lr_scheduler(self, num_warmup_steps):
        if self.config.optim.lr_scheduler == 'constant':
            self.lr_scheduler = get_constant_schedule_with_warmup(optimizer=self.optimizer,
                                                                  num_warmup_steps=num_warmup_steps)
        elif self.config.optim.lr_scheduler == 'cosine':
