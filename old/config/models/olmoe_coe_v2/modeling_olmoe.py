...
class OlmoeSparseMoeBlock(nn.Module):
    def __init__(self, config):
        ...
        if self.use_igate:
            self.gate = nn.ModuleList([nn.Linear(config.hidden_size, self.num_experts, bias=False) for _ in range(self.inner_iter)])
        else:
    ...
    def forward(
        ...
        **kwargs,
        ...
        def custom_forward(hidden_states, _iter):
            ...
            mlp_output = self.mlp(hidden_states, _iter)
            if self.use_moe:
        ...
        for _iter in range(self.inner_iter):
            if self.use_checkpoint:
                hidden_states, router_logits = torch.utils.checkpoint.checkpoint(
                    custom_forward,
                    hidden_states,
                    _iter,
                    use_reentrant=False # TODO: check if this is correct
                )
            else:
                hidden_states, router_logits = custom_forward(hidden_states, _iter)
        ...
        # router_logits = torch.concat(all_router_logits, dim=0) # TODO: check if this is correct
