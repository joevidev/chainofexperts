        ...
        **kwargs,
        ...
        Args:
            ...
            kwargs (`dict`, *optional*):
                ...
                into the model
        """
        # Initialize variables to store outputs across iterations
        all_self_attns = () if output_attentions else None
        all_router_logits = () if output_router_logits else None
        present_key_value = None
        
        # Save initial input for final residual connection if needed
        original_hidden_states = hidden_states
        
        # Repeat the inner forward function for N iterations
        for i in range(self.inner_iter):
            # Pass the current state to inner_forward
            outputs = self._inner_forward(
                hidden_states=hidden_states,
                attention_mask=attention_mask,
                position_ids=position_ids,
                past_key_value=past_key_value if i == 0 else present_key_value,
                output_attentions=output_attentions,
            ...
            # Update cache for next iteration if using cache
            if use_cache:
                present_key_value = outputs[1 if not output_attentions else 2]
            
            # Collect router logits if needed
            if output_router_logits:
                router_logits_index = 1
                if output_attentions:
                    router_logits_index += 1
                if use_cache:
                    router_logits_index += 1
                all_router_logits += (outputs[router_logits_index],)
        
        # Prepare final outputs tuple
